...
let read_expression state expr =
  match expr with
  | Register r -> read_register state r  (* Prebere vrednost iz registra *)
  | Const n -> n  (* Vrne vrednost konstante *)
...
let perform_unop op state reg =
  let value = match reg with
    | A -> state.a
    | B -> state.b
    | C -> state.c
    | D -> state.d
  in
  let new_value = op value in
  write_register state reg new_value
...
let perform_binop op state reg expr =
  let value1 = match reg with
    | A -> state.a
    | B -> state.b
    | C -> state.c
    | D -> state.d
  in
  let value2 = match expr with
    | Register r -> (match r with
        | A -> state.a
        | B -> state.b
        | C -> state.c
        | D -> state.d)
    | Const n -> n
  in
  let new_value = op value1 value2 in
  write_register state reg new_value
...
let next (Address addr) = Address (addr + 1)
...
let jump state (Address address) =
  { state with ip = Address address }
...
let compare state n1 n2 =
  let zero = n1 = n2 in
  let carry = n1 < n2 in
  { state with zero = zero; carry = carry }
...
let conditional_jump state address condition =
  if condition then jump state address else proceed state
...
let return state =
  let addr, state' = pop_stack state in
  jump state' (Address addr)
...
let run_instruction st = function
  | MOV (reg, exp) -> perform_binop (fun x _ -> x) st reg exp |> proceed
  (* | MOV (reg, exp) -> 
    let value = read_expression st exp in
    write_register st reg value |> proceed *)
  | ADD (reg, exp) -> perform_binop ( + ) st reg exp |> proceed
  | SUB (reg, exp) -> perform_binop ( - ) st reg exp |> proceed
  | INC reg -> perform_unop succ st reg |> proceed
  | DEC reg -> perform_unop pred st reg |> proceed
  | MUL (reg, exp) -> perform_binop ( * ) st reg exp |> proceed
  | DIV exp -> perform_binop ( / ) st A exp |> proceed
  (* | DIV exp -> 
    let value = read_expression st exp in
    perform_binop ( / ) st A (Const value) |> proceed *)
  (* Pozor, OCaml land/lor/lxor interpretira kot simbole, zato jih pišemo infiksno! *)
  | AND (reg, exp) -> perform_binop ( land ) st reg exp |> proceed
  | OR (reg, exp) -> perform_binop ( lor ) st reg exp |> proceed
  | XOR (reg, exp) -> perform_binop ( lxor ) st reg exp |> proceed
...
let run_program state =
  let rec loop st =
    match st.ip with
    | Address address when address >= Array.length st.instructions || address < 0 -> st
    | Address address ->
        let instruction = st.instructions.(address) in
        if instruction = HLT then st
        else loop (run_instruction st instruction)
  in
  loop state
